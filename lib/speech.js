...
log.setLevel("warn");
var service = cfenv.getAppEnv().getService('speech_to_text_vtiwari4')

if(service == null) {
  service = {'credentials':{'username':'7305c69e-8e55-4200-93c9-b25b1f001928', 'password':'gexfEXMvv2Zg'}};
}
...
var convert_speech_to_text = function (audio, cb) {
  var params = {
    ...
    content_type: 'audio/wav; rate=8000',
  ...
  tmp.file({postfix: '.wav'}, function _tempFileCreated (err, input, fd) {
    ...
    tmp.file({postfix: '.wav'}, function _tempFileCreated (err, output, fd) {
      ...
      save_to_file(url, input, function () {
      	convert_speech_to_text(input, cb)
